...
import Notification from '~/components/UNotification.vue'
import JournalPages from '~/components/JournalPage.vue'
...
<template>
  <div class="relative min-h-screen overflow-hidden bg-[#fff9f0]">
    ...
    <main class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-20">
      ...
      <header class="text-center mb-8 sm:mb-12">
        ...
        <h1 class="text-2xl sm:text-4xl font-bold text-base-content mb-2">Commence ton journal</h1>
        <p class="text-base sm:text-lg text-base-content opacity-80">
          Exprime ton humeur pour voir ton évolution au fil du temps
        </p>
      </header>
      <div :class="history.length ? 'grid gap-6 sm:gap-8 md:gap-10 lg:grid-cols-2' : 'flex justify-center'">
        <div :class="history.length ? 'bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-white/20' : 'bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-white/20 max-w-md w-full'">
        ...
        >
          <h2 class="text-xl sm:text-2xl font-bold text-base-content mb-6 text-center">Évolution de ton humeur</h2>
        ...
        <div v-if="history.length" class="lg:col-span-2">
          <JournalPages :entries="history" @clear="clearHistory" />
        </div>
      </div>
    </main>
  </div>
