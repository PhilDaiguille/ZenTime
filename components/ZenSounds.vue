...
<template>
  <section class="my-8 text-center">
    <p class="text-sm opacity-70 mb-6 max-w-2xl mx-auto">
      Vous pouvez choisir un son de fond pour accompagner votre écriture et créer une atmosphère apaisante.
    </p>
    ...
    <div class="flex flex-wrap justify-center gap-5 mb-10">
      ...
      >
    <span class="text-sm sm:text-base font-medium text-base-content opacity-90">
      {{ sound.name }}
    </span>
      </button>
    </div>
    ...
    >
      ...
      <button
          @click="stop"
          class="px-3 py-1 rounded-full text-sm shadow transition-all
           bg-gradient-to-r from-red-100 to-red-200 hover:from-red-200 hover:to-red-300
           border border-red-300 text-red-800 opacity-90 hover:opacity-100"
      >
        Stop
      </button>
    </div>
